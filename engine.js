...
const CANON_PATH = path.join(CARL_DIR, 'markers_canonical.json');
...
function computeInputHash(payload) {
  ...
  const segments = Array.isArray(payload.segments) ? payload.segments.map(s => ({ who: s.who, text: toNFC(s.text) })) : null;
...
function stableSegmentFromText(text) {
  ...
  for (const line of lines) {
    ...
    if (m) {
      ...
      const who = whoRaw.startsWith('a') ? 'A' : whoRaw.startsWith('b') ? 'B' : 'other';
      ...
      if (mk.type === 'DETECT' || mk.type === 'ATO' || mk.type === 'SEM') {
        if (mk.regex) {
          ...
          while ((m = reAll.exec(str)) !== null) {
            ...
            out.push({ id: mk.id, type: mk.type, span: { start, end }, segment_idx: segIdx, who: seg.who, evidence: m[0] || '' });
  ...
  if (promoMap?.map?.length) {
    for (const r of promoMap.map) {
      if (r.promote && Array.isArray(r.from_ATO_ids)) {
        rules.push({
          target: r.promote,
          when: { segment_co_occurs: { ids: r.from_ATO_ids, min: r.when?.segment_co_occurs?.min_ATO || 2 } },
        ...
        for (const semId of semSet) {
          ...
          if (any) {
            for (const add of r.annotate || []) {
              ann.push({ type: add.type, id: add.id, segment_idx: segIdx, who: 'other' });
            }
          }
        }
      }
...
function computeFeatures(canon, counts, density) {
  ...
  const caps = canon.globals?.text_caps || { ATO: 100, SEM: 100, CLU: 100, MEMA: 100 };
  function scale(n, cap) { return clamp01(n / cap); }
...
function dot(w, f) { return w.ATO * f.ATO + w.SEM * f.SEM + w.CLU * f.CLU + w.MEMA * f.MEMA; }
...
function gateOutput(markersAll, segments) {
  const enoughMarkers = markersAll.filter(m => ['ATO','SEM','CLU','MEMA'].includes(m.type)).length >= 5;
  const enoughSegs = (segments?.length || 0) >= 2;
  return !(enoughMarkers && enoughSegs);
}

function uniqId(prefix, n) { return prefix + '_' + (n.toString(36)); }
...
function arrangeMarkersForOutput(detected, promotions, annotations) {
  ...
  for (const p of promotions) {
    outputMarkers.push({ id: p.sem_id, type: 'SEM', span: { start: 0, end: 0 }, segment_idx: p.segment_idx, who: p.who, promotion_of: p.from_atos, evidence: '' });
  }
  for (const a of annotations) {
    outputMarkers.push({ id: a.id, type: a.type, span: { start: 0, end: 0 }, segment_idx: a.segment_idx, who: a.who, promotion_of: [], evidence: '' });
  }
...
function modeAdjustments(mode, weights) {
  ...
  function scaleAll(mult) {
    for (const k of ['trust','deesc','conflict','sync']) {
      ...
      w.ATO *= mult.ATO; w.SEM *= mult.SEM; w.CLU *= mult.CLU; w.MEMA *= mult.MEMA;
    }
  }
  ...
  if (modes && modes[key]) {
    ...
    if (conf.calib_adjust) {
      const mu = conf.calib_adjust.mu; const sigma = conf.calib_adjust.sigma;
    ...
    switch (key) {
      ...
      case 'learn':
        ...
        for (const k of ['trust','deesc','conflict','sync']) { clone.calib.mu[k] = 0.5; clone.calib.sigma[k] = 0.25; }
        break;
      case 'free':
...
export function run(input, options = {}) {
  ...
  if (typeof normalized.text === 'string') normalized.text = toNFC(normalized.text);
  if (Array.isArray(normalized.segments)) normalized.segments = normalized.segments.map(s => ({ who: s.who, text: toNFC(s.text) }));
  ...
  const defaultWeights = (weightsFile && weightsFile.indices && weightsFile.calib)
    ? weightsFile
    : (Array.isArray(weightsFile?.examples) && weightsFile.examples[0])
      ? weightsFile.examples[0]
      : null;
  if (!defaultWeights) throw new Error('Gewichte nicht gefunden: weights.json enthält weder Root noch examples[0]');
  ...
  const out = {
    meta: { input_hash, canon_hash, engine_hash, elapsed_ms: Date.now() - t0, gated },
    segments,
    markers,
...
// Exporte für Tests
export const __internals = { detect_all, applyPromotion, annotateClustersAndMemory, computeCounts, computeDensity, computeFeatures, computeIndices, gateOutput, modeAdjustments };
